...
var CustomError, Err_Cannot_find_a_running_iterator_with_given_ID, Err_Only_string_can_be_the_RPC_method_name, Err_Cannot_call_method_starts_with_rpc_dot_directly, Err_Then_is_accessed_on_local_implementation_Please_explicitly_mark_if_it_is_thenable_in_the_options, Messages, makeHostedMessage, errors, DOMExceptionHeader, RecoverError, removeStackHeader, globalDOMException, isString, isBoolean, isFunction, isObject, ERROR, undefined$1, Promise_resolve, isArray, replayFunction, jsonrpc, Request, SuccessResponse, ErrorResponse, ErrorResponseParseError, ErrorResponseInvalidRequest, ErrorResponseMethodNotFound, ErrorResponseMapped, defaultErrorMapper, isJSONRPCObject, toString, deleteUndefined, deleteFalsy, i, AsyncCallIgnoreResponse, AsyncCallNotify, AsyncCallBatch, generateRandomID, undefinedToTrue, normalizeLogOptions, normalizeStrictOptions, isEventBasedChannel, isCallbackBasedChannel;
var init_base = __esm({
  "../../node_modules/async-call-rpc/out/base.mjs"() {
    ...
    DOMExceptionHeader = "DOMException:";
    RecoverError = (type, message, code, stack) => {
      try {
...
${stack}`);
    ...
    removeStackHeader = (stack) => String(stack).replace(/^.+\n.+\n/, "");
    globalDOMException = () => {
      try {
        return DOMException;
      } catch (e) {
    ...
    isObject = (params) => typeof params === "object" && params !== null;
    ERROR = "Error";
    undefined$1 = void 0;
    Promise_resolve = (x) => Promise.resolve(x);
    isArray = Array.isArray;
    replayFunction = () => "() => replay()";
    jsonrpc = "2.0";
    Request = (id, method, params, remoteStack) => {
      const x = {
        jsonrpc,
        id,
        method,
    ...
    ErrorResponseParseError = (e, mapper) => {
    ...
    ErrorResponseInvalidRequest = (id) => ErrorResponse(id, -32600, "Invalid Request");
    ErrorResponseMethodNotFound = (id) => ErrorResponse(id, -32601, "Method not found");
    ErrorResponseMapped = (request, e, mapper) => {
      const { id } = request;
      const { code, message, data } = mapper(e, request);
      return ErrorResponse(id, code, message, data);
    };
    defaultErrorMapper = (stack = "", code = -1) => (e) => {
    ...
    i = "AsyncCall/";
    AsyncCallIgnoreResponse = Symbol.for(i + "ignored");
    AsyncCallNotify = Symbol.for(i + "notify");
    AsyncCallBatch = Symbol.for(i + "batch");
    generateRandomID = () => Math.random().toString(36).slice(2);
    undefinedToTrue = (x) => x === void 0 ? true : x;
    ...
    isEventBasedChannel = (x) => "send" in x && isFunction(x.send);
    isCallbackBasedChannel = (x) => "setup" in x && isFunction(x.setup);
  }
});
...
__export(electron_exports, {
  appInfo: () => appInfo,
  getElectronAPIs: () => getElectronAPIs
});
...
function getMainAPIs() {
  const meta = (() => {
    const val = process.argv.find((arg) => arg.startsWith("--main-exposed-meta="))?.split("=")[1];
  ...
  const events = (() => {
    ...
    import_electron.ipcRenderer.setMaxListeners(100);
...
function getHelperAPIs() {
  const events$ = new import_rxjs.Subject();
  const meta = (() => {
    const val = process.argv.find((arg) => arg.startsWith("--helper-exposed-meta="))?.split("=")[1];
    return val ? JSON.parse(val) : null;
  })();
  ...
  const rpc = AsyncCall(rendererToHelperServer, {
    channel: helperPort$.then(
      (helperPort) => createMessagePortChannel(helperPort)
    ),
    log: false
  });
  const toHelperHandler = (namespace, name) => {
    return rpc[`${namespace}:${name}`];
  };
  const toHelperEventSubscriber = (namespace, name) => {
    return (callback) => {
  ...
  const setup = (meta2) => {
    const { handlers, events: events2 } = meta2;
    ...
    const helperEvents = Object.fromEntries(
      events2.map(([namespace, eventNames]) => {
        return [
          namespace,
          Object.fromEntries(
            eventNames.map((name) => {
...
var import_electron, import_rxjs, appInfo, helperPort$, createMessagePortChannel;
var init_electron = __esm({
  "../../packages/infra/src/preload/electron.ts"() {
    ...
    helperPort$ = new Promise(
      (resolve) => import_electron.ipcRenderer.on("helper-connection", (e) => {
...
// src/preload/bootstrap.ts
var import_electron2 = require("electron");
(async () => {
  const { appInfo: appInfo2, getElectronAPIs: getElectronAPIs2 } = await Promise.resolve().then(() => (init_electron(), electron_exports));
  const { apis, events } = getElectronAPIs2();
  import_electron2.contextBridge.exposeInMainWorld("appInfo", appInfo2);
  import_electron2.contextBridge.exposeInMainWorld("apis", apis);
  import_electron2.contextBridge.exposeInMainWorld("events", events);
  import_electron2.contextBridge.exposeInMainWorld("platform", process.platform);
  const globals = {
    ipcRenderer: {
      send(channel, ...args) {
      ...
      once(channel, listener) {
        import_electron2.ipcRenderer.once(channel, listener);
        return this;
      },
  ...
  try {
    import_electron2.contextBridge.exposeInMainWorld("affine", globals);
  } catch (error) {
